...
[ApiController]
public class CenzorsController :ControllerBase , ICenzors
{
  private readonly AppDbContext _dbContext;
    ...
    public CenzorsController(AppDbContext dbContext, IMapper mapper,
    ...
    {
        var request = await _dbContext.Cenzors
            .FirstOrDefaultAsync(sr => sr.Id == Id.Value);
        if (request != null)
        {
            _dbContext.Cenzors.Remove(request);
    ...
    {
        return _mapper.Map<IEnumerable<ComisionModel>>(await _dbContext.Cenzors.Include(x => x.FileNam)
    ...
    public async Task<ComisionModel> GetById(Guid? Id)
    {
        return _mapper.Map<ComisionModel>(
            await _dbContext.Cenzors.FirstOrDefaultAsync(sr => sr.Id == Id.Value));
    }
    ...
    public async Task AddorUpdate(ComisionModel model)
    {
        ...
        ) // if id is null then its a new record
        {
            ...
            _dbContext.Cenzors.Add(_mapper.Map<Cenzor>(model));
        }
        else
        {
            var request = await _dbContext.Cenzors
